...
int main() {
	...
	size_t d = 10; // 16 electron, 8 electron pairs

	auto hamiltonian = xerus::TTOperator(std::vector<size_t>(2*d,4)); // TT operator initialized with 0
...
//	}
	...
	auto phi = xerus::TTTensor::random(std::vector<size_t>(d,4),std::vector<size_t>(d - 1,20));
