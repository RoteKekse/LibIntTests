...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const auto filename = (argc > 1) ? argv[1] : "hartree-fock/h2o.xyz";
    ...
    for (auto s=0; s<shells.size(); ++s){
      nao += shells[s].size();
    	cout << "\tshell size = " << shells[s].size() << endl;
    }
    cout << "\tnao = " << nao << endl;
...
std::vector<libint2::Shell> make_sto3g_basis(const std::vector<Atom>& atoms) {
  ...
  for(auto a=0; a<atoms.size(); ++a) {
    ...
    switch (atoms[a].atomic_number) {
      case 1: // Z=1: hydrogen
        std::cout << "\tatoms = " << atoms[a].atomic_number << std::endl;
        shells.push_back(
            {
